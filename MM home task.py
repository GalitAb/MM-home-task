...
class team_played(Resource):    
    ...
    def get(self,home_team): # GET request
        ...
        return jsonify({'data': data_fount})        
...
class AllTeams(Resource):
    def __init__(self):
        path = r'\C:\Users\galit\Desktop\api'
        self.csv_files = glob.glob(os.path.join(path, "*.csv"))
        display(self.csvfiles)
        #self.data = pd.read_csv('result_played.csv')      
    def get(self,home_team): # GET request
...
api.add_resource(team_played, '/team-played/<string:home_team>')
api.add_resource(team_upcoming, '/team-upcoming/<string:home_team>')
